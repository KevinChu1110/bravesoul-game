...
static func _village() -> Dictionary:
	var m := _base("翠谷村 · 夜（本村）", Color(0.08, 0.04, 0.05), 2800, 1400, Vector2(880, 1060), "village")
	m["entities"] = [
		_e("hut_a", 448, 645, 80, 72, "焦黑茅屋", Color(0.35, 0.25, 0.2), true),
		_e("hut_b", 816, 590, 72, 64, "塌半的倉", Color(0.4, 0.28, 0.2), true),
		_e("hut_c", 1139, 1203, 70, 60, "半毀木屋", Color(0.38, 0.26, 0.22), true),
		_e("maisui", 1332, 996, 48, 64, "麥穗", Color(0.85, 0.55, 0.45)),
		_e("well", 1470, 924, 52, 52, "井沿", Color(0.4, 0.42, 0.45), true),
		_e("sword", 1290, 858, 48, 48, "鏽劍", Color(0.55, 0.5, 0.4)),
		_e("ash_pile", 1698, 964, 44, 40, "灰堆", Color(0.3, 0.28, 0.28)),
		_e("fire", 1860, 878, 56, 56, "火光", Color(0.9, 0.35, 0.1), true),
		_e("cart", 1116, 756, 60, 52, "翻倒的車", Color(0.45, 0.35, 0.25), true),
		_e("fence_row", 556, 976, 200, 28, "燒焦籬笆", Color(0.3, 0.25, 0.2), true),
		_e("field_west", 414, 1117, 120, 48, "西邊田埂", Color(0.35, 0.3, 0.2)),
		_e("sign_east", 2482, 756, 44, 52, "東路木牌", Color(0.4, 0.35, 0.25)),
		_e("exit_east", 2510, 784, 72, 80, "往東 · 荒路", Color(0.3, 0.25, 0.2)),
		_e("exit_outskirts", 298, 884, 72, 64, "村外田野", Color(0.35, 0.4, 0.3)),
		_e("to_cave", 2066, 444, 64, 56, "山邊洞窟", Color(0.3, 0.28, 0.32)),
		_e("to_grave", 2402, 1088, 64, 56, "村後墓園", Color(0.35, 0.32, 0.3)),
		_e("shrine_stub", 1626, 465, 48, 56, "村口小祠", Color(0.45, 0.4, 0.35), true),
		_e("orchard", 1972, 1215, 56, 48, "枯果園", Color(0.4, 0.35, 0.28)),
	]
	return m


static func _village_outskirts() -> Dictionary:
	var m := _base("翠谷 · 村外田野", Color(0.07, 0.08, 0.06), 2600, 1200, Vector2(690, 896), "village")
	m["entities"] = [
		_e("back_village", 363, 768, 56, 56, "回村子", Color(0.4, 0.35, 0.3)),
		_e("scare_field", 653, 936, 48, 56, "稻草人", Color(0.4, 0.35, 0.25)),
		_e("pond", 1040, 824, 80, 48, "乾涸池塘", Color(0.3, 0.35, 0.4), true),
		_e("woodpile", 870, 616, 56, 40, "木柴堆", Color(0.45, 0.35, 0.25)),
		_e("to_mill", 1425, 400, 64, 64, "風車田", Color(0.4, 0.42, 0.35)),
		_e("trail_east", 2292, 688, 72, 64, "接荒路", Color(0.35, 0.4, 0.3)),
		_e("windmill", 1726, 396, 60, 80, "風車殘架", Color(0.4, 0.38, 0.35), true),
		_e("stone_circle", 1676, 1016, 56, 48, "石圈", Color(0.42, 0.4, 0.38)),
	]
	return m
...
static func _town() -> Dictionary:
	var m := _base("騎士堡壘 · 外城廣場", Color(0.1, 0.11, 0.14), 3200, 1500, Vector2(792, 1100), "town")
	var flag_label := "旗幟（兔爪）" if GameState.has_flag("c1_flag_paw") else "灰旗"
	m["entities"] = [
		_e("greybeard", 1024, 751, 48, 64, "灰鬚", Color(0.55, 0.55, 0.6)),
		_e("wall_notice", 1442, 710, 44, 52, "告示牆", Color(0.45, 0.4, 0.35), true),
		_e("message_stone", 736, 853, 48, 52, "留言石", Color(0.5, 0.55, 0.7), true),
		_e("to_market", 888, 1254, 64, 56, "下城市集", Color(0.5, 0.4, 0.3)),
		_e("market", 1096, 868, 64, 52, "攤位殘架", Color(0.5, 0.4, 0.3), true),
		_e("ding", 848, 991, 48, 64, "釘釘·鐵匠", Color(0.7, 0.4, 0.25)),
		_e("forge_sign", 978, 958, 44, 44, "鐵匠招牌", Color(0.55, 0.4, 0.25)),
		_e("fountain", 1832, 988, 64, 52, "乾涸水池", Color(0.4, 0.45, 0.5), true),
		_e("flag", 1760, 699, 48, 56, flag_label, Color(0.85, 0.75, 0.35), true),
		_e("star", 2304, 856, 48, 64, "星讀", Color(0.45, 0.5, 0.75)),
		_e("bench", 2176, 974, 48, 36, "石凳", Color(0.45, 0.45, 0.48)),
		_e("sprout", 1616, 1328, 48, 64, "小芽", Color(0.55, 0.75, 0.45)),
		_e("silk", 2224, 721, 48, 64, "絲絨·書吏", Color(0.55, 0.5, 0.65)),
		_e("to_barracks_yard", 792, 1179, 64, 56, "演武場", Color(0.4, 0.38, 0.4)),
		_e("barracks", 676, 1088, 72, 64, "舊兵營", Color(0.4, 0.38, 0.4), true),
		_e("chapel", 1544, 676, 64, 72, "小禮拜堂", Color(0.45, 0.42, 0.48), true),
		_e("to_sewers", 1356, 1390, 56, 48, "下水道口", Color(0.3, 0.32, 0.3)),
		_e("gate_arch", 1116, 676, 56, 72, "內城門影", Color(0.4, 0.38, 0.42), true),
		_e("exit_keep", 1236, 690, 72, 72, "進入內城", Color(0.45, 0.4, 0.35)),
		_e("exit_wild", 2820, 390, 72, 72, "出城荒野", Color(0.35, 0.4, 0.3)),
		_e("exit_world", 668, 1014, 56, 56, "六域地圖", Color(0.4, 0.5, 0.55)),
		_e("menu_save", 1040, 1337, 48, 48, "存檔石", Color(0.4, 0.45, 0.5)),
		_e("stable", 2296, 1126, 64, 56, "空馬廄", Color(0.42, 0.36, 0.3), true),
	]
	return m


static func _town_keep() -> Dictionary:
	var m := _base("騎士堡 · 內城與內殿道", Color(0.09, 0.09, 0.12), 2600, 1200, Vector2(690, 968), "town")
	m["entities"] = [
		_e("back_town", 571, 972, 56, 56, "回外城廣場", Color(0.4, 0.4, 0.45)),
		_e("hall", 936, 560, 80, 72, "騎士舊廳", Color(0.4, 0.38, 0.42), true),
		_e("armor", 1177, 564, 40, 56, "空盔甲架", Color(0.5, 0.5, 0.55)),
		_e("throne_hall", 1421, 550, 72, 64, "議政廳門", Color(0.45, 0.4, 0.35), true),
		_e("keep_well", 1264, 800, 48, 48, "內井", Color(0.4, 0.42, 0.48), true),
		_e("statue_knight", 679, 688, 48, 64, "無名騎士像", Color(0.5, 0.48, 0.5), true),
		_e("archive", 1741, 588, 56, 56, "檔案庫門", Color(0.42, 0.4, 0.45), true),
		_e("silk", 1810, 712, 48, 64, "絲絨·書吏", Color(0.55, 0.5, 0.65)),
		_e("codex_shelf", 1862, 656, 48, 48, "典籍架", Color(0.45, 0.4, 0.5), true),
		_e("exit_wild_inner", 1044, 1096, 72, 64, "側門·荒野", Color(0.35, 0.4, 0.3)),
		_e("to_leo_court", 2309, 324, 64, 56, "內殿前院道", Color(0.5, 0.4, 0.3)),
	]
	return m
