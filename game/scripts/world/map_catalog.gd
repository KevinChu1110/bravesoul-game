...
static func _village() -> Dictionary:
	var m := _base("翠谷村 · 夜（本村）", Color(0.08, 0.04, 0.05), 2800, 1563, Vector2(880, 1174), "village")
	m["entities"] = [
		_e("hut_a", 448, 719, 80, 72, "焦黑茅屋", Color(0.35, 0.25, 0.2), true),
		_e("hut_b", 816, 657, 72, 64, "塌半的倉", Color(0.4, 0.28, 0.2), true),
		_e("hut_c", 1139, 1341, 70, 60, "半毀木屋", Color(0.38, 0.26, 0.22), true),
		_e("maisui", 1332, 1110, 48, 64, "麥穗", Color(0.85, 0.55, 0.45)),
		_e("well", 1470, 1028, 52, 52, "井沿", Color(0.4, 0.42, 0.45), true),
		_e("sword", 1290, 954, 48, 48, "鏽劍", Color(0.55, 0.5, 0.4)),
		_e("ash_pile", 1698, 1072, 44, 40, "灰堆", Color(0.3, 0.28, 0.28)),
		_e("fire", 1860, 977, 56, 56, "火光", Color(0.9, 0.35, 0.1), true),
		_e("cart", 1116, 841, 60, 52, "翻倒的車", Color(0.45, 0.35, 0.25), true),
		_e("fence_row", 556, 1084, 200, 28, "燒焦籬笆", Color(0.3, 0.25, 0.2), true),
		_e("field_west", 414, 1243, 120, 48, "西邊田埂", Color(0.35, 0.3, 0.2)),
		_e("sign_east", 2482, 841, 44, 52, "東路木牌", Color(0.4, 0.35, 0.25)),
		_e("exit_east", 2510, 875, 72, 80, "往東 · 荒路", Color(0.3, 0.25, 0.2)),
		_e("exit_outskirts", 298, 985, 72, 64, "村外田野", Color(0.35, 0.4, 0.3)),
		_e("to_cave", 2066, 493, 64, 56, "山邊洞窟", Color(0.3, 0.28, 0.32)),
		_e("to_grave", 2402, 1212, 64, 56, "村後墓園", Color(0.35, 0.32, 0.3)),
		_e("shrine_stub", 1626, 516, 48, 56, "村口小祠", Color(0.45, 0.4, 0.35), true),
		_e("orchard", 1972, 1353, 56, 48, "枯果園", Color(0.4, 0.35, 0.28)),
	]
	return m
...
static func _town() -> Dictionary:
	var m := _base("騎士堡壘 · 外城廣場", Color(0.1, 0.11, 0.14), 3200, 1786, Vector2(792, 1294), "town")
	...
	m["entities"] = [
		...
		_e("message_stone", 736, 1010, 48, 52, "留言石", Color(0.5, 0.55, 0.7), true),
		_e("to_market", 888, 1489, 64, 56, "下城市集", Color(0.5, 0.4, 0.3)),
		_e("market", 1096, 1028, 64, 52, "攤位殘架", Color(0.5, 0.4, 0.3), true),
		_e("ding", 848, 1177, 48, 64, "釘釘·鐵匠", Color(0.7, 0.4, 0.25)),
		_e("forge_sign", 978, 1134, 44, 44, "鐵匠招牌", Color(0.55, 0.4, 0.25)),
		_e("fountain", 1832, 1171, 64, 52, "乾涸水池", Color(0.4, 0.45, 0.5), true),
		_e("flag", 1760, 828, 48, 56, flag_label, Color(0.85, 0.75, 0.35), true),
		_e("star", 2304, 1016, 48, 64, "星讀", Color(0.45, 0.5, 0.75)),
		_e("bench", 2176, 1151, 48, 36, "石凳", Color(0.45, 0.45, 0.48)),
		_e("sprout", 1616, 1578, 48, 64, "小芽", Color(0.55, 0.75, 0.45)),
		_e("silk", 2224, 855, 48, 64, "絲絨·書吏", Color(0.55, 0.5, 0.65)),
		_e("to_barracks_yard", 792, 1399, 64, 56, "演武場", Color(0.4, 0.38, 0.4)),
		_e("barracks", 676, 1292, 72, 64, "舊兵營", Color(0.4, 0.38, 0.4), true),
		_e("chapel", 1544, 803, 64, 72, "小禮拜堂", Color(0.45, 0.42, 0.48), true),
		_e("to_sewers", 1356, 1649, 56, 48, "下水道口", Color(0.3, 0.32, 0.3)),
		...
		_e("exit_world", 668, 1203, 56, 56, "六域地圖", Color(0.4, 0.5, 0.55)),
		_e("menu_save", 1040, 1586, 48, 48, "存檔石", Color(0.4, 0.45, 0.5)),
		_e("stable", 2296, 1336, 64, 56, "空馬廄", Color(0.42, 0.36, 0.3), true),
	]
	return m
...
static func _cross_north() -> Dictionary:
	var m := _base("岔路 · 北山道", Color(0.09, 0.12, 0.1), 2400, 1350, Vector2(200, 609), "cross_north")
	m["entities"] = [
		_e("back_cross", 100, 582, 56, 56, "回岔路", Color(0.45, 0.5, 0.4)),
		_e("switchback", 600, 446, 64, 48, "之字坡", Color(0.4, 0.42, 0.38)),
		_e("pine_row", 900, 334, 80, 56, "松林", Color(0.3, 0.45, 0.35), true),
		_e("wayshrine", 1300, 560, 48, 56, "山神小祠", Color(0.45, 0.42, 0.4), true),
		_e("path_dojo_c", 1900, 448, 72, 64, "通往道場", Color(0.4, 0.55, 0.35)),
		_e("cloud_view", 1600, 277, 56, 48, "雲海眺望", Color(0.5, 0.55, 0.6)),
	]
	return m


static func _cross_east() -> Dictionary:
	var m := _base("岔路 · 東塔荒原道", Color(0.1, 0.09, 0.14), 2600, 1462, Vector2(200, 638), "cross_east")
	m["entities"] = [
		_e("back_cross", 100, 630, 56, 56, "回岔路", Color(0.45, 0.5, 0.4)),
		_e("dead_trees", 600, 457, 72, 56, "枯樹陣", Color(0.35, 0.3, 0.35), true),
		_e("ash_wind", 1000, 651, 56, 40, "灰風帶", Color(0.4, 0.35, 0.4)),
		_e("to_blackflame_scar", 1400, 590, 64, 56, "黑焰疤地", Color(0.35, 0.2, 0.4)),
		_e("path_tower_c", 2100, 633, 72, 64, "塔下方向", Color(0.45, 0.35, 0.55)),
		_e("watch_rock", 1700, 388, 48, 48, "眺望岩", Color(0.4, 0.38, 0.42)),
	]
	return m
...
static func _dojo() -> Dictionary:
	var m := _base("武鬥道場 · 山門院落", Color(0.12, 0.14, 0.1), 2800, 1563, Vector2(200, 705), "dojo")
	var ents: Array = [
		_e("acha", 350, 598, 48, 64, "阿茶", Color(0.7, 0.55, 0.4)),
		_e("gate_bell", 600, 333, 48, 64, "山門鐘", Color(0.55, 0.5, 0.35)),
		_e("training_dummy", 500, 837, 40, 56, "木人樁", Color(0.5, 0.4, 0.3)),
		_e("scroll_wall", 800, 294, 48, 48, "拳譜牆", Color(0.45, 0.42, 0.35), true),
		_e("stone_garden", 1100, 534, 56, 44, "石庭", Color(0.4, 0.45, 0.4)),
		_e("tea", 1000, 775, 48, 48, "茶席", Color(0.5, 0.4, 0.3)),
		_e("to_dojo_inner", 1400, 476, 64, 56, "內院", Color(0.4, 0.5, 0.35)),
		_e("to_dojo_bamboo", 1600, 957, 64, 56, "竹林徑", Color(0.35, 0.5, 0.35)),
		_e("to_dojo_peak", 1900, 356, 64, 56, "山巔道", Color(0.4, 0.48, 0.4)),
		_e("trial_hall", 2100, 603, 80, 88, "試煉堂·阿波", Color(0.35, 0.5, 0.3)),
		_e("dorm", 1300, 1077, 64, 56, "僧寮", Color(0.4, 0.38, 0.32), true),
		_e("save_c3", 140, 1256, 48, 48, "存檔石", Color(0.4, 0.45, 0.5)),
		_e("back_mist", 100, 414, 56, 48, "回霧隱村", Color(0.45, 0.5, 0.6)),
		_e("exit_cross_d", 2500, 1199, 72, 64, "六域岔路", Color(0.45, 0.5, 0.4)),
	]
	if GameState.has_flag("boss.abo_cleared"):
		ents.append(_e("path_forest", 1800, 1319, 72, 64, "林道·遊俠", Color(0.35, 0.55, 0.4)))
		ents.append(_e("path_tower", 2550, 658, 72, 64, "向塔之路", Color(0.4, 0.3, 0.5)))
	m["entities"] = ents
	return m


static func _dojo_inner() -> Dictionary:
	var m := _base("道場 · 內院與靜室", Color(0.11, 0.13, 0.1), 2200, 1238, Vector2(200, 538), "dojo_inner")
	m["entities"] = [
		_e("back_dojo", 100, 527, 56, 56, "回山門", Color(0.4, 0.5, 0.35)),
		_e("zen_pond", 600, 488, 80, 48, "靜心池", Color(0.35, 0.45, 0.5), true),
		_e("scripture", 900, 366, 48, 56, "經閣", Color(0.45, 0.4, 0.35), true),
		_e("meditation", 1200, 611, 56, 48, "打坐席", Color(0.4, 0.42, 0.35)),
		_e("master_room", 1600, 489, 64, 56, "掌門靜室", Color(0.42, 0.38, 0.32), true),
	]
	return m


static func _dojo_bamboo() -> Dictionary:
	var m := _base("道場 · 竹林徑", Color(0.08, 0.14, 0.09), 2400, 1350, Vector2(200, 595), "dojo_bamboo")
	m["entities"] = [
		_e("back_dojo", 100, 584, 56, 56, "回山門", Color(0.4, 0.5, 0.35)),
		_e("bamboo_wall", 600, 426, 80, 64, "竹牆", Color(0.3, 0.5, 0.35), true),
		_e("stream_stone", 1000, 605, 48, 40, "溪石", Color(0.4, 0.45, 0.4)),
		_e("hidden_spar", 1400, 484, 56, 48, "隱切磋場", Color(0.4, 0.48, 0.35)),
		_e("leaf_pile", 1700, 726, 48, 36, "落葉堆", Color(0.4, 0.45, 0.3)),
		_e("to_dojo_peak", 2000, 424, 64, 56, "上山巔", Color(0.4, 0.48, 0.4)),
	]
	return m


static func _dojo_peak() -> Dictionary:
	var m := _base("道場 · 山巔試煉台", Color(0.12, 0.14, 0.16), 2200, 1228, Vector2(200, 534), "dojo_peak")
	m["entities"] = [
		_e("back_dojo", 100, 523, 56, 56, "下山", Color(0.4, 0.5, 0.35)),
		_e("peak_platform", 800, 489, 100, 72, "試煉台", Color(0.45, 0.42, 0.38), true),
		_e("wind_flag", 600, 363, 40, 56, "風旗", Color(0.5, 0.45, 0.35)),
		_e("trial_hall", 1500, 518, 80, 88, "巔上·阿波影", Color(0.35, 0.5, 0.3)),
		_e("sunrise_view", 1800, 361, 56, 48, "日出眺", Color(0.6, 0.5, 0.4)),
	]
	return m
...
static func _coast() -> Dictionary:
	var m := _base("維京海岸 · 碼頭與岸道", Color(0.12, 0.16, 0.2), 3200, 1786, Vector2(180, 794), "coast")
	var ents: Array = [
		_e("tide_roar", 350, 708, 48, 64, "潮吼", Color(0.65, 0.45, 0.35)),
		_e("dock", 600, 951, 64, 52, "碼頭鎮", Color(0.4, 0.45, 0.5)),
		_e("to_coast_harbor", 800, 1021, 64, 56, "深港", Color(0.4, 0.48, 0.55)),
		_e("boat_wreck", 500, 1154, 64, 44, "破船骸", Color(0.4, 0.35, 0.3)),
		_e("forge_c5", 1000, 541, 48, 56, "岸邊鍛爐", Color(0.7, 0.4, 0.25)),
		_e("net_rack", 900, 741, 48, 40, "漁網架", Color(0.45, 0.4, 0.35)),
		_e("cliff_path", 1400, 675, 56, 48, "峭壁道", Color(0.45, 0.42, 0.4)),
		_e("runestone", 1600, 431, 48, 56, "符文石", Color(0.5, 0.48, 0.55), true),
		_e("to_coast_cave", 1800, 953, 64, 56, "潮汐洞", Color(0.35, 0.4, 0.45)),
		_e("to_coast_wreck", 2000, 1159, 64, 56, "沉船灣", Color(0.4, 0.38, 0.35)),
		_e("cliff_gate", 2200, 747, 64, 56, "崖上祭壇道", Color(0.5, 0.4, 0.35)),
		_e("boar_cliff", 2700, 690, 80, 88, "石拳崖", Color(0.6, 0.4, 0.3)),
		_e("save_c5", 140, 1431, 48, 48, "存檔石", Color(0.4, 0.45, 0.5)),
		_e("back_forest", 100, 469, 56, 48, "回森林", Color(0.35, 0.5, 0.4)),
		_e("exit_cross_c", 2900, 1368, 72, 64, "六域岔路", Color(0.45, 0.5, 0.4)),
	]
	if GameState.has_flag("boss.stonefist_cleared"):
		ents.append(_e("path_tower_c5", 2500, 1368, 72, 64, "向塔之路", Color(0.45, 0.35, 0.55)))
	m["entities"] = ents
	return m


static func _coast_cliff() -> Dictionary:
	var m := _base("崖上 · 祭壇與浪聲", Color(0.1, 0.13, 0.16), 2200, 1228, Vector2(200, 534), "coast")
	m["entities"] = [
		_e("back_coast", 100, 523, 56, 56, "回碼頭", Color(0.4, 0.5, 0.55)),
		_e("altar", 700, 387, 64, 56, "浪祭壇", Color(0.5, 0.45, 0.4), true),
		_e("beacon", 1000, 328, 40, 64, "烽火台", Color(0.55, 0.4, 0.3), true),
		_e("boar_cliff_near", 1600, 516, 72, 80, "石拳崖近路", Color(0.6, 0.4, 0.3)),
		_e("gull_nest", 1300, 604, 40, 36, "海鷗巢", Color(0.5, 0.48, 0.45)),
	]
	return m


static func _coast_harbor() -> Dictionary:
	var m := _base("海岸 · 深港碼頭", Color(0.1, 0.14, 0.18), 2600, 1451, Vector2(200, 648), "coast_harbor")
	m["entities"] = [
		_e("back_from_coast_sub", 100, 624, 56, 56, "回岸道", Color(0.4, 0.5, 0.55)),
		_e("longship", 700, 600, 100, 56, "長船", Color(0.4, 0.35, 0.3), true),
		_e("crane", 1100, 482, 48, 72, "吊桿", Color(0.45, 0.42, 0.4), true),
		_e("warehouse", 1400, 660, 72, 56, "貨倉", Color(0.4, 0.38, 0.35), true),
		_e("tide_gauge", 1700, 537, 40, 48, "潮位尺", Color(0.4, 0.45, 0.5)),
		_e("captain_post", 2000, 598, 56, 48, "船長柱", Color(0.5, 0.4, 0.35)),
	]
	return m


static func _coast_cave() -> Dictionary:
	var m := _base("海岸 · 潮汐洞", Color(0.08, 0.1, 0.14), 2200, 1238, Vector2(200, 538), "coast_cave")
	m["entities"] = [
		_e("back_from_coast_sub", 100, 527, 56, 56, "出洞", Color(0.4, 0.5, 0.55)),
		_e("tide_pool", 600, 611, 72, 48, "潮池", Color(0.3, 0.4, 0.5), true),
		_e("crystal", 1000, 426, 40, 48, "海晶", Color(0.4, 0.55, 0.65)),
		_e("pirate_mark", 1300, 548, 48, 40, "海盜標記", Color(0.45, 0.35, 0.3)),
		_e("air_hole", 1600, 362, 40, 40, "氣孔光", Color(0.5, 0.55, 0.6)),
		_e("deep_water", 1800, 673, 64, 48, "深水", Color(0.25, 0.3, 0.4), true),
	]
	return m


static func _coast_wreck() -> Dictionary:
	var m := _base("海岸 · 沉船灣", Color(0.1, 0.12, 0.14), 2400, 1340, Vector2(200, 592), "coast_wreck")
	m["entities"] = [
		_e("back_from_coast_sub", 100, 579, 56, 56, "回岸道", Color(0.4, 0.5, 0.55)),
		_e("hull", 700, 486, 120, 72, "船體殘骸", Color(0.4, 0.35, 0.3), true),
		_e("mast", 1000, 365, 40, 80, "斷桅", Color(0.42, 0.38, 0.32), true),
		_e("chest_half", 1300, 661, 48, 40, "半埋箱", Color(0.5, 0.4, 0.25)),
		_e("skull_rock", 1600, 541, 48, 48, "頭骨岩", Color(0.45, 0.42, 0.4)),
		_e("wreck_boss", 1750, 489, 80, 88, "沉船船長影", Color(0.45, 0.55, 0.7)),
		_e("to_coast_cave", 1900, 724, 56, 48, "洞口（內側）", Color(0.35, 0.4, 0.45)),
	]
	return m
