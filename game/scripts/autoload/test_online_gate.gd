...
##   3. RPC 結果解析正確 —— PostgREST 有時把結果包成陣列，讀錯就整個經濟打不動。
##
...
func _check_offline_safe(gate: Node) -> void:
	...
	gate.post_message("town_gate", "測試", collect)
	gate.candle_increment(collect)
	gate.leaderboard_submit("rift_weekly", 100, collect)

	if results.size() != 4:
		gate.offline_only = was_offline
		_fail("純單機下 4 支寫入 API 只回了 %d 個結果（有 API 沒有回呼）" % results.size())
		return
	for r in results:
	...
	if ledger.is_empty() or int(ledger[0].get("gold", -1)) != 0:
		...
		return
	print("  ok 純單機下 4 支寫入 API 安全回失敗，餘額查詢回 0")
