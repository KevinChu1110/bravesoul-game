...
func _render() -> void:
	...
	var body := "選一格接著走。每一格是各自獨立的一段旅途，不會互相蓋掉。\n\n"
	var buttons: Array = []
	...
	buttons.append({"text": "返回", "cb": _close})
	_host.ui_panel("旅途紀錄", body, buttons)
...
func _slot_line(s: Dictionary) -> String:
	...
	if not bool(s.get("used", false)):
		return "%s[b]第 %d 格[/b] [color=#8a8070]空著[/color]\n\n" % [mark, n]

	var line := "%s[b]第 %d 格[/b] %s · Lv%d · %s" % [
		mark, n, str(s.get("name", "")), int(s.get("level", 1)), str(s.get("place", "")),
	]
	if int(s.get("ng_plus", 0)) > 0:
		line += " · 黑焰迴響 ×%d" % int(s.get("ng_plus", 0))
...
func _add_slot_buttons(buttons: Array, s: Dictionary) -> void:
	var n := int(s.get("slot", 0))
	if not bool(s.get("used", false)):
		buttons.append({"text": "第 %d 格 · 新的旅途" % n, "cb": start_new.bind(n)})
		return
	if bool(s.get("future", false)):
		## 讀下去只會把它降級寫壞，所以連按鈕都不給
		buttons.append({"text": "第 %d 格 · 讀不了" % n, "cb": _explain_future.bind(n)})
	else:
		buttons.append({"text": "第 %d 格 · 接著走" % n, "cb": load_slot.bind(n)})
	var label := ("第 %d 格 · 再按一次就真的刪掉" % n) if _pending_delete == n else ("第 %d 格 · 刪掉" % n)
	buttons.append({"text": label, "cb": delete_slot.bind(n)})


func load_slot(slot: int) -> void:
	if SaveManager.load_game(slot) != OK:
		_host.ui_toast("第 %d 格讀不起來。" % slot)
		_render()
		return
	_host.ui_toast("回到第 %d 格的旅途。" % slot)
...
func start_new(slot: int) -> void:
	...
	if SaveManager.save_game(slot) != OK:
		_host.ui_toast("第 %d 格寫不進去。" % slot)
		_render()
		return
	_host.ui_toast("第 %d 格，新的旅途。" % slot)
	if on_loaded.is_valid():
		on_loaded.call()
...
func delete_slot(slot: int) -> void:
	...
	if SaveManager.delete_slot(slot) != OK:
		_host.ui_toast("第 %d 格刪不掉。" % slot)
	else:
		_host.ui_toast("第 %d 格已經清空。" % slot)
	_render()


func _explain_future(slot: int) -> void:
	_host.ui_toast("第 %d 格是更新的遊戲存的，這個版本讀了會弄壞它。" % slot)
