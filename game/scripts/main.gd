...
func _go_equip_panel() -> void:
	...
	var body: String = EquipmentSystem.status_bbcode()
	body += "\n\n爆擊 %.1f%% · 暴傷 +%.0f%% · 傷害浮動 ±%.0f%%" % [
		GameState.effective_crit(), GameState.effective_crit_dmg(), GameState.effective_variance() * 100.0
	]
	var buttons: Array = []
	for s in EquipmentSystem.SLOTS:
		...
		if uid != "":
			...
			match s:
				...
				"accessory":
					sn = "卸下飾品"
			buttons.append({"text": sn, "cb": _equip_unequip.bind(s)})
	var n := 0
	for e in GameState.equip_bag:
		if n >= 8:
			break
		var u := str(e.get("uid", ""))
		buttons.append({"text": "裝上 " + EquipmentSystem.label(e), "cb": _equip_wear.bind(u)})
		n += 1
	buttons.append({"text": "試產一件（測試）", "cb": _equip_debug_drop})
	buttons.append({"text": "倉庫", "cb": _go_warehouse_panel})
	buttons.append({"text": "返回", "cb": _hub_back})
	_panel("裝備", body, buttons)
